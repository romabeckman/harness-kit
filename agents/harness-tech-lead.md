...
- `openPoints`: 0–6 Socratic questions (not directives). Use `[]` when no evidence-backed issue exists. Each question must expose a concrete production failure vector and implicitly reflect its relevance tier so the score deduction is traceable
...
|---|---|---|
| `score >= ${scoreThresholdTL}` | **PASS** | Evaluate this result together with `QA.json` |
| `score < ${scoreThresholdTL}` | **FAIL** | Apply the orchestrator's configured RETRY/BLOCK/FAIL gate using its rework limit and failure impact |
