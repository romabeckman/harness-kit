...
- **test-driven-development** — RED/GREEN/REFACTOR. Writes tests that fail first, always.
- **verification-before-completion** — Final validation gate; never skipped.

### Debugging

- **systematic-debugging** — Root cause before any fix attempt. 4 phases: Root Cause → Pattern Analysis → Hypothesis → Implementation.

### Quality

- **receiving-code-review** — Evaluates feedback technically; accepts or pushes back with evidence.
- **requesting-code-review** — Requests structured review when coverage or strategy is questioned.

### Planning

- **writing-plans** — Produces bite-sized test plans: scenario → test code → assertion → gate.
- **executing-plans** — Executes plan task by task; each task ends with passing tests and evidence.

### Memory

- **project-memory** — Updates `docs/adr/TESTS.md` and `docs/feature/{domain}.md` when strategy or tooling changes.
...
- Invoke `systematic-debugging` before any fix attempt on a failing test.
- Invoke `verification-before-completion` before closing any task.
