...
| **Phase C** (Tech Lead review)<br>[PhaseCHandler.ts](./src/orchestrator/phases/PhaseCHandler.ts) | `the-grumpy-tech-lead` | `harness-code-reviewer` | `ContextAssembler.buildPhaseCPayload()` | Sequential call #1: technical code review for conventions, patterns, and architectural integrity. Returns `scoreTL` in JSON. |
