...
export class RefinementHandler extends AbstractPhaseHandler {
  ...
  private async generateQuestions(context: Reviewontext, scope: string): Promise<RefinementQuestion[]> {
    ...
    const prompt = [
      `## Objective`,
      `Analyze the project scope below and generate 5-8 Socratic questions that surface`,
      `architectural decisions, technical risks, edge cases, constraints, and quality requirements`,
      `that are implicit or missing from the scope.`,
      ``,
      `Write the output to: \`${questionsPath}\``,
      ``,
      `For each question, provide:`,
      `- A clear, specific question`,
      `- A recommended answer based on your analysis of the scope`,
      `- Brief context explaining why this question matters`,
      ``,
      `## Output Format`,
      `Return ONLY a single fenced JSON array with objects matching this schema:`,
      `\`\`\`json`,
      `[`,
      ...
      ``,
      `## Rules`,
      `- Focus on decisions that will impact the development plan`,
      `- Prioritize: architecture > security > performance > maintainability`,
      `- Recommendations should be opinionated and justified`,
      `- Questions should be answerable by a developer who knows the project`,
      ``,
      `<scope>`,
      `\`\`\`markdown`,
      scope.trim(),
