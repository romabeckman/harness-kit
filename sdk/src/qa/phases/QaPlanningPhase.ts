...
const ACTIONS = ['navigate', 'click', 'fill', 'press', 'wait'] as const
...
export class QaPlanningPhase implements QaPhaseHandler {
  ...
  private parsePlan(raw: string, request: QaPhaseContext['request']): QaPlan {
    ...
    const target = stringValue(data.target) ?? request.target
    ...
    const scenarios = scenarioData.map((value, index) => this.parseScenario(value, profile as QaProfile, index))
  ...
  private parseScenario(value: unknown, planProfile: QaProfile, index: number): QaScenario {
    if (!isRecord(value)) throw new Error(`Invalid agentic QA plan: scenario ${index + 1} must be an object`)
    const id = stringValue(value.id)
    const profile = value.profile
    const criterionIds = stringArray(value.criterionIds)
    ...
    else scenario.actions = this.parseActions(value.actions, index)
    return scenario
  }
  ...
  private parseActions(value: unknown, index: number): QaBrowserAction[] {
    ...
    return value.map((action, actionIndex) => {
      ...
      return { type: action.type as QaBrowserAction['type'], selector: stringValue(action.selector), value: stringValue(action.value) }
    })
  }
}
