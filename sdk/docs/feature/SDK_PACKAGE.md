...
## OVERVIEW
The `sdk_package` configures `package.json` and `tsconfig.build.json` so `npm publish` emits a clean, type-safe CJS-only package. Release `1.6.0` publishes SDK version `0.8.2`; the exports map exposes one entry and restricts the tarball to compiled files.
