...
- **Developer Session Lifecycle**: Developer session created during initial implementation is preserved in `context.developerSession`. On rework retry (`reworks > 0`), the orchestrator resumes the developer session with a concise continuation prompt (addressing `REWORK-LOG.md` findings without re-sending full specs). Reviews run with clean, isolated sessions. The developer session is discarded on `PASS`, `BLOCK`, `FAIL`, `--skip-validation`, or feature transition.
...
REQUIRED: On development retries with an active matching developer session, resume the session with a concise continuation prompt containing `REWORK-LOG.md` and pending tasks; fall back to standalone prompt when no session exists.
REQUIRED: Review phases (Tech Lead and Adversarial QA) MUST run with clean, independent sessions and must never participate in or receive the developer session continuation.
