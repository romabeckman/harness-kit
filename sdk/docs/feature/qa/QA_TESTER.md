...
---
```graph
{"node_id":"feature:qa_tester","domain":"qa","implements":["adr:architecture"],"tested_by":["adr:tests"],"entrypoints":["src/qa/QaAgenticOrchestrator.ts","src/cli/services/qa-service.ts","src/cli/services/run-service.ts"],"registration_files":["src/cli/run.ts","src/cli/utils/constants.ts","src/cli/services/qa/QaOrchestratorFactory.ts","src/index.ts","src/qa/index.ts"],"reference_files":["src/qa/engine/CurlDriver.ts","src/qa/auth/QaAuthConfigStore.ts","src/qa/auth/types.ts","src/cli/services/qa/QaAuthCommand.ts"],"code_files":["src/cli/utils/run-args-parser.ts","src/cli/services/qa/types.ts","src/cli/services/qa/QaArgsParser.ts","src/cli/services/qa/QaDevelopmentRenewal.ts","src/cli/services/qa/QaExploratoryCommand.ts","src/cli/services/qa/QaReportOutput.ts","src/qa/services/QaService.ts","src/qa/services/QaDeveloperReportGenerator.ts","src/qa/services/QaExploratoryService.ts","src/qa/services/QaExecutionMemory.ts","src/qa/services/QaPlanValidator.ts","src/qa/services/QaRuntimeManager.ts","src/qa/services/QaTargetProbe.ts","src/qa/types.ts","src/qa/progress.ts","src/qa/services/QaRunStore.ts","src/qa/services/QaVerdictPolicy.ts","src/qa/engine/PlaywrightDriver.ts","src/qa/engine/MobileWebDriver.ts","src/qa/engine/AccessibilityDriver.ts","src/qa/engine/McpClientDriver.ts","src/qa/engine/CliDriver.ts","src/qa/engine/WebSocketDriver.ts","src/qa/engine/QaAuthRedaction.ts","src/qa/engine/index.ts","src/qa/services/index.ts","src/qa/ui/QaTerminalView.ts","src/qa/utils/QaAgentFileOutput.ts","src/qa/utils/QaPlanRetargeting.ts","src/qa/phases/types.ts","src/qa/phases/QaPlanningPhase.ts","src/qa/phases/QaValidationPhase.ts","src/qa/phases/QaExecutionPhase.ts","src/qa/phases/QaAnalysisPhase.ts","src/qa/phases/QaReportingPhase.ts","src/qa/phases/index.ts"],"test_files":["src/qa/__tests__/PlaywrightDriverIsolation.test.ts","src/qa/auth/__tests__/QaAuthConfigStore.test.ts","src/qa/auth/__tests__/QaAuthExecution.test.ts","src/qa/__tests__/QaArchitecture.test.ts","src/qa/__tests__/QaExtendedEngines.test.ts","src/qa/__tests__/QaAuthEngineSecurity.test.ts","src/qa/__tests__/QaAgenticOrchestrator.test.ts","src/qa/__tests__/QaService.test.ts","src/qa/__tests__/QaImprovements.test.ts","src/qa/__tests__/QaCurlRegressions.test.ts","src/qa/__tests__/QaRunStore.test.ts","src/qa/__tests__/QaReviewRegressions.test.ts","src/qa/services/__tests__/QaPlanValidator.test.ts","src/qa/services/__tests__/QaTargetProbe.test.ts","src/qa/ui/__tests__/QaTerminalView.test.ts","src/cli/services/__tests__/qa-service.test.ts","src/cli/services/__tests__/run-service.test.ts","src/cli/utils/__tests__/run-args-parser.test.ts"],"knowledge":{"schema_version":1,"entities":[{"id":"capability:agentic-qa-execution","type":"capability","label":"Agentic QA execution","definition":"Plan, validate, execute, analyze, and report runtime QA scenarios.","aliases":[]},{"id":"rule:qa-adaptive-analysis","type":"rule","label":"Adaptive analysis opt-in","definition":"Adaptive QA analysis is disabled unless explicitly enabled.","aliases":[]}],"claims":[{"id":"claim:qa-phase-pipeline","subject":"capability:agentic-qa-execution","relation":null,"object":null,"statement":"QaAgenticOrchestrator defaults to planning, validation, execution, analysis, and reporting handlers; report generation defaults on and adaptive analysis defaults off.","kind":"observation","status":"supported","evidence":[{"kind":"code","source":"src/qa/QaAgenticOrchestrator.ts","locator":"constructor default phase handlers and reportEnabled/analysisEnabled initialization","snapshot":null}],"derived_from":[],"gap":null},{"id":"claim:qa-analysis-opt-in","subject":"capability:agentic-qa-execution","relation":"constrained_by","object":"rule:qa-adaptive-analysis","statement":"QaAgenticOrchestrator sets analysisEnabled to options.analysis ?? false.","kind":"observation","status":"supported","evidence":[{"kind":"code","source":"src/qa/QaAgenticOrchestrator.ts","locator":"constructor: analysisEnabled initialization","snapshot":null}],"derived_from":[],"gap":null}]}}
```
...
3. **Validate** plan, target, profile, scenarios, and driver prerequisites; correct plan-contract errors once, then block invalid requests.
...
Each supplied `--scenario` is mandatory. Preserve its stable ID and require an executable mapping. Instructions define actor, preconditions, state, action, and observable outcome; assert business behavior when transport success is insufficient.

## REPORT OUTPUT

Run `hrns qa report --run <id> --output <format>`; supported formats are `json`, `html`, `markdown`, and `send-to-developer` (LLM-generated scope). Omit `--output` for selection (default: `json`).
...
Ask before sending **FAILED** or **BLOCKED** scenarios to development (default `false`); skip reports, noninteractive terminals, and actionless runs. Preserve `--agent`/`--debug`; use `hrns run --reset --run <id>`. Never combine `--run` with `--scope`.
...
Coverage counts reconcile planned scenarios; only passed or failed observations count as tested. Unplanned categories are omitted.
...
Legacy **Streamable HTTP** initializes a session and reuses its returned session and protocol-version headers. Stateless `2026-07-28` requests carry protocol, client, capability, method, and tool headers. CLI/MCP calls have 30-second deadlines; WebSocket exchanges have five-second deadlines. Accessibility checks cover image alternatives, form labels, and document language, not full conformance. Target probes check reachability; drivers own HTTP status assertions.
...
- [**SDK_CLI.md**](../cli/../cli/sdk_cli.md): CLI registration and command conventions.
