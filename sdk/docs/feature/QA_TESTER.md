...
3. **Validate** strict JSON loaded from the planner output file, profiles, targets, and executable scenarios.
...
QA planning, analysis, and reporting require the agent to write one JSON document to a temporary `docs/qa/` handoff file. Each phase reads its file, then removes the temporary artifact; persisted plans and reports remain under `docs/qa/`.
...
REQUIRED: Apply HTTP credentials to same-origin API, MCP, and browser traffic. Send Curl request configuration through transient stdin, never process arguments. Redact exact resolved values from Curl/MCP/CLI evidence. Scope Basic browser credentials to target origin; apply cookies through Playwright context APIs. Inject only explicitly mapped `environment` values into CLI child processes.

REQUIRED: Block authenticated CLI scenarios without explicit environment mappings. Block all authenticated WebSocket scenarios until a header-capable adapter exists.
