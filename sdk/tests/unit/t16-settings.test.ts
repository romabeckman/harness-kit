...
describe('T16 — HarnessSettings', () => {
  ...
  it('generates default global config if it does not exist', () => {
    ...
    const resolved = settings.resolve('claude-code', 'bootstrap')
  ...
  it('merges project config over global config', () => {
    ...
    expect(settings.resolve('claude-code', 'bootstrap')).toEqual({
      model: 'claude-3-5-sonnet-latest',
      effort: 'high'
    })

    // Overridden resolves to project
    expect(settings.resolve('claude-code', 'phase_a')).toEqual({
  ...
  it('handles missing runner or phase gracefully by returning empty object', () => {
    ...
    expect(settings.resolve('claude-code', 'non-existent-phase')).toEqual({})
  })
})
